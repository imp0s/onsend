...
import { base64ToUint8Array, removeMetadataAndComments, uint8ArrayToBase64 } from '../src/docCleanup';

async function buildDocx(withComments = true): Promise<Uint8Array> {
  const zip = new JSZip();
...
describe('removeMetadataAndComments', () => {
  it('strips metadata and comments from docx files', async () => {
    const sample = await buildDocx(true);
  ...
  it('leaves files unchanged when nothing is removable', async () => {
    const sample = await buildDocx(false);
    const cleaned = await removeMetadataAndComments(sample);
